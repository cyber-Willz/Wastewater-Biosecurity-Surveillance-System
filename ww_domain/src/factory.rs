...
pub struct SiteParams {
    ...
    pub name: String,
    pub region: String,
    /// Estimated population whose waste passes through this site.
...
pub struct SampleParams {
    ...
    /// ID of the site where the sample was collected.
    pub site_id: String,
    /// Daily volumetric flow at time of sampling (litres).
    pub flow_liters: f64,
    pub qc_passed: bool,
    pub notes: String,
}

/// Parameters for a single pathogen / AMR signal detected in a sample.
pub struct SignalParams {
    pub signal_id: Option<String>,
    /// ID of the sample this signal was extracted from.
    pub sample_id: String,
    /// Human-readable pathogen name, e.g. `"SARS-CoV-2"`.
    pub pathogen: String,
    /// PCR / sequencing target, e.g. `"N1"`, `"E6L"`, `"ctxA"`.
    pub target_gene: String,
    /// Log₁₀ copies per litre (flow-normalised).
    pub log10_copies_per_l: f64,
    /// Detection method, e.g. `"ddPCR"`, `"qPCR"`, `"metagenomics"`.
    pub method: String,
}
...
pub struct AlertParams {
    ...
    pub severity: String,
    /// ID of the signal that triggered this alert.
    pub signal_id: String,
    /// EWMA z-score that crossed the detection threshold.
    pub z_score: f64,
    /// Spectral network anomaly score in `[0, 1]`.
...
impl DomainFactory {
    /// Create and persist a [`MonitoringSite`].
    ///
    /// Returns the assigned `site_id`.
    ...
    /// Create and persist a [`PathogenSignal`], automatically linking it to
    /// its parent sample.
    ///
